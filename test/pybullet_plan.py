...
urdf_filename = os.path.join(root_folder_path, 'robot', 'abb_crb15000_support', 'urdf', 'crb15000_5_95.urdf')
srdf_filename = os.path.join(root_folder_path, 'robot', 'abb_crb15000_support', 'srdf', 'abb_crb15000_5_95.srdf')
...
tool_json_path = os.path.join(root_folder_path, 'tool', 'otto1', 'otto1.json')
...
planning_problem_path = os.path.join('test', 'design', 'planning_problem_0_test.json')
        ...
        for robotic_movement in pp.get_robotic_movements():
            ...
            if robotic_movement.target_frame is not None:
                ...
                if plane_rotation_angle is not None:
                    tcp_frame = rotate_frame(robotic_movement.target_frame, plane_rotation_angle)
                else:
                ...
                # Compute IK
                configuration = client.inverse_kinematics(robot, target_frame, group=None, options={'avoid_collisions': False})
                # print ("Task [%s] %s" % (robotic_movement.task_id, robotic_movement.tag))    
                # print ("IK Result:" , configuration) 
                # Check Collision
                if configuration is not None:
                    reachable_count += 1
                    cc_result = client.check_collisions(robot, configuration, options={'diagnosis':diagnose})
                    if not cc_result: collisionfree_count += 1
            else:
                # Movements with target configuration are assumed to be reachable
                reachable_count += 1
                # Check Collision for target configuration
                cc_result = client.check_collisions(robot, robotic_movement.target_configuration, options={'diagnosis':diagnose})
                if not cc_result: collisionfree_count += 1
    return total_count, reachable_count, collisionfree_count

# total_count, reachable_count, collisionfree_count = check_pp_ik(robot, client, tool, pp, plane_rotation_angle = None, diagnose=False)
# print ("%i Movements, %i Reachable, %i CollisionFree" % (total_count, reachable_count, collisionfree_count))
...
def check_pp_with_global_ik_rotation(robot, client, tool, pp, plane_rotation_steps = 36, diagnose=False):
    ...
    for i in range(plane_rotation_steps):
        ...
        total_count, reachable_count, collisionfree_count = check_pp_ik(robot, client, tool, pp, plane_rotation_angle = angle_rad, diagnose=False)
        print ("Rotation(%.1f degrees) : %i Movements, %i Reachable, %i CollisionFree" % (angle, total_count, reachable_count, collisionfree_count))
        if total_count == collisionfree_count:
            possible_angles.append(angle)
    return possible_angles

# global_rotation_angle = check_pp_with_global_ik_rotation(robot, client, tool, pp, plane_rotation_steps = 20, diagnose=False)
...
def test_all_angles_for_a_frame(client, robot, tool, robotic_movement, starting_config, tries = 1, plane_rotation_steps = 36):
    ...
    for i in range(plane_rotation_steps):
        ...
        target_frame = tool.from_tcf_to_t0cf([tcp_frame])[0]
        # Compute IK
        configuration = random_ik(client, robot, target_frame, starting_config=starting_config, tries = tries, collision = True, visualize = False)
...
possible_angles, planned_configurations = test_all_angles_for_a_frame(
    client, robot, tool, first_movement, starting_config, tries = 20, plane_rotation_steps = plane_rotation_steps)
...
def dfs(all_movements, currentStep, planned_configurations=None):
    ...
    if currentStep >= len(all_movements):  # Define a function to check if current step is goal
    ...
    possible_configurations = [c for c in possible_configurations if c.close_to(planned_configurations[-1], jump_tolerance)]
    # random.shuffle(possible_configurations)
    print ("Step: %i has %i options"%(currentStep, len(possible_configurations)))
