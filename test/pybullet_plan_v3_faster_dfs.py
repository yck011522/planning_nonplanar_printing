...
jump_tolerance = 0.7
ik_num_tries = 4  # Number of IK tries for each frame Typically 1
# ---------------------
...
# planning_problem_file_name = "240207_PathPlanning_BioPrint901_Panel_V2.json"
planning_problem_file_name = "241002_T1.json"
...
def test_frame_IK(
    ...
    possible_configurations = random_ik(
        client,
        robot,
        flange_frame,
    ...
    possible_configurations = [c for c in possible_configurations if c.close_to(
        starting_config, jump_tolerance)]
    return possible_configurations[0] if possible_configurations else None
...
def dfs(client, robot, tool, all_movements, current_step, initial_config, frame_generators=[]):
    ...
    while True:
        ...
        tries = 16 if current_step == 0 else ik_num_tries
