...
avoid_collision = False  # Normally True, False only for debugging
...
urdf_filename = os.path.join(
    root_folder_path, "robot", "ur10", "urdf", "ur10.urdf"
)
srdf_filename = os.path.join(
    root_folder_path, "robot", "ur10", "srdf", "ur10.srdf"
)
...
planning_problem_file_name = "240814_T1_VL.json"
...
def test_frame_IK(
    ...
    print("Testing Frame IK for TCP Frame: ", tcp_frame)
    print("                 for flange Frame: ", flange_frame)
    return random_ik(
        client,
        robot,
        flange_frame,
...
def dfs(client, robot, tool, all_movements, current_step, initial_config, frame_generators=[]):
    ...
    while True:
        ...
        if frame_option is None and current_step == 0:
            print("Search Failed at Step %i" % current_step)
            return None
        ...
        tries = 40 if current_step == 0 else 10
        ...
        if result_config is not None:
            ...
            print("Creating generator for step %i, frame %s" %
                  (current_step + 1, next_target_frame))
...
def plan_all_movements_with_initial_config(pp, client, robot, tool, movements, plane_rotation_steps_first_target=10):
    ...
    print("Creating generator for step 0, frame %s" %
          first_movement.target_frame)
